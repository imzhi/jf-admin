        ...
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content clearfix">
                    <div class="row">
                        <div class="col-md-3">
                            <a href="{{ route('admin::show.manageuser.create') }}" class="btn btn-w-m btn-default">新增</a>
                        </div>
                    </div>
                    <form action="{{ route('admin::show.manageuser.list') }}">
                        <div class="row">
                            <div class="col-md-3">
                        ...
                        <table class="table table-bordered table-hover golden-table">
                            ...
                            <tbody>
                                ...
                                <tr data-id="{{ $item->id }}">
                                    ...
                                    <td>{{ $item->login_ip }}</td>
                                    <td>{{ $status_rels[$item->status] ?? __('admin.unknow') }}</td>
                                    <td>
                                        <button type="button" class="btn btn-default status-btn" data-status="{{ 1 - $item->status }}">{{ $status_rels[1 - $item->status] ?? __('admin.unknow') }}</button>
                                        <a href="{{ route('admin::show.manageuser.create', [$item->id]) }}" class="btn btn-default">编辑</a>
                                        <a href="{{ route('admin::show.manageuser.distribute', [$item->id]) }}" class="btn btn-default">分配角色</a>
                                    </td>
                                </tr>
                                @empty
                                <tr>
...
<script>
    $('.status-btn').click(function() {
        ...
        layer.confirm('确定要' + text + '该账号吗？', function (layerIndex) {
            ...
            $.ajax({
                url: '{{ route('admin::manageuser.status') }}',
